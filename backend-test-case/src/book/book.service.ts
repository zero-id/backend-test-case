...
export class BookService {
  ...
  async returnBook(payload: BorrowReturnBookDto) {
    ...
    const returnDate = new Date("2025-01-01T00:00:00.000Z");
    ...
    const updateBorrow = await this.prisma.borrow.update({
      where: {
        id: borrow.id,
      },
      data: {
    ...
    if (borrowDuration > 7) {
      ...
      return {
        message: `Book returned successfully, member ${member.name} (${member.code}) has a penalty!`,
        penaltyEnds,
        data: updateBorrow,
      };
    }
